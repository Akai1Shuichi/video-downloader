...
def test_format_sort_prefers_resolution_then_compatible_mp4_codecs() -> None:
    result = select_format("720")

    assert result.sort[:2] == ["res", "fps"]
    assert result.sort[2:] == ["vcodec:h264", "acodec:aac", "ext:mp4:m4a"]
